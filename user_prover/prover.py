...
def provador():
    try:
        ...
        ip_server_vpn = "http://127.0.0.1:5000"
        ...
        inicio_etapa = time.time()
        response = requests.post(inicio_interacao_url, json=data)
        ...
        if response.status_code == 200:
            ...
            response = requests.post(verify_url, json={'resposta_pi': resposta_pi, 'interacao': interacao})
            ...
            if response.json().get('status') == 1:
                ...
                for i in range(2):
                    ...
                    inicio_etapa = time.time()
                    response = requests.post(multiplas_rodadas_url, json=data)
